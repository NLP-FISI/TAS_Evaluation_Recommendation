...
# URLs de las imágenes (directamente en el HTML)
IMAGENES = {
    "guacamayo": "https://i.imgur.com/GVoqssd.jpg",
    "mono": "https://i.imgur.com/MvaNJgE.jpg"
}

# Contenido HTML con placeholders para las imágenes
CONTENIDO_HTML = """<div class="afiche-animales">
  ...
  <div class="seccion-secuestrados">
    <div class="animal-card">
      ...
      <img src="{{guacamayo}}" alt="Guacamayo escarlata" class="animal-imagen">
    ...
    <div class="animal-card">
      <h3 class="etiqueta-secuestrado">Secuestrado</h3>
      <img src="{{mono}}" alt="Mono Fraile" class="animal-imagen">
...
try:
    ...
    if texto:
        ...
        contenido_final = CONTENIDO_HTML
        for clave, url in IMAGENES.items():
            contenido_final = contenido_final.replace(f"{{{{{clave}}}}}", url)
        
        texto.contenido = contenido_final
        if not texto.titulo:
            texto.titulo = "Tráfico de animales silvestres"
        
        db.commit()
        print("✓ Texto actualizado con imágenes")
        print(f"  ID: {texto.id_texto}")
        print(f"  Título: {texto.titulo}")
        print(f"  Formato: HTML con imágenes embebidas")
        print(f"  URLs: {IMAGENES}")
    else:
